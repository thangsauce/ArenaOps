...
const initialNotifications: Notification[] = [
  {
    ...
    message: "Jamie Nguyen did not appear for Match 4 in Spring Valorant Open.",
  ...
  },
  {
    ...
    type: "delay",
    title: "Match delayed",
    message: "Match 3 (Sam Torres vs Taylor Park) delayed by 15 min.",
  ...
  },
  {
    ...
    message: "Alex Kim confirmed availability for Morning and Afternoon slots.",
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const [delayedMatchTimers, setDelayedMatchTimers] = useState<
    Record<string, DelayedMatchTimer>
  >({});
  ...
  const updateMatch = useCallback(
    (tournamentId: string, matchId: string, updates: Partial<Match>) => {
      setTournaments((prev) =>
        prev.map((t) =>
          t.id !== tournamentId
            ? t
  ...
  const updateTournament = useCallback(
    (tournamentId: string, updates: Partial<Tournament>) => {
      setTournaments((prev) =>
        prev.map((t) => {
          if (t.id !== tournamentId) return t;

          const isLockedScheduleUpdate =
            (t.status === "completed" || t.status === "active") &&
            (Object.prototype.hasOwnProperty.call(updates, "venueLocationId") ||
              Object.prototype.hasOwnProperty.call(
                updates,
                "selectedTimeBlockId",
              ));

          if (isLockedScheduleUpdate) return t;
  ...
  const updateTournamentStatus = useCallback(
    (tournamentId: string, status: Tournament["status"]) => {
      setTournaments((prev) =>
        prev.map((t) => {
          if (t.id !== tournamentId) return t;
  ...
  const removeParticipant = useCallback(
    (tournamentId: string, participantId: string) => {
      setTournaments((prev) =>
        prev.map((t) =>
          t.id !== tournamentId
            ? t
  ...
  const removeParticipants = useCallback(
    (tournamentId: string, participantIds: string[]) => {
        ...
        prev.map((t) => {
          ...
          return {
            ...
            matches: t.matches.map((match) => {
              const participant1Removed = match.participant1Id
                ? removedIds.has(match.participant1Id)
                : false;
              const participant2Removed = match.participant2Id
                ? removedIds.has(match.participant2Id)
                : false;
              const winnerRemoved = match.winnerId
                ? removedIds.has(match.winnerId)
                : false;
              if (
                !participant1Removed &&
                !participant2Removed &&
              ...
              return {
                ...match,
                participant1Id: participant1Removed
                  ? null
                  : match.participant1Id,
                participant2Id: participant2Removed
                  ? null
                  : match.participant2Id,
                winnerId: null,
                score1:
                  participant1Removed || participant2Removed ? 0 : match.score1,
                score2:
                  participant1Removed || participant2Removed ? 0 : match.score2,
                status:
                  participant1Removed || participant2Removed
                    ? "scheduled"
                    : match.status,
              };
            }),
  ...
  const addParticipants = useCallback(
    (tournamentId: string, participantsToAdd: Participant[]) => {
  ...
  const startMatch = useCallback(
    (tournamentId: string, matchId: string) => {
      ...
      const p1 =
        t?.participants.find((p) => p.id === m?.participant1Id)?.name ?? "TBD";
      const p2 =
        t?.participants.find((p) => p.id === m?.participant2Id)?.name ?? "TBD";
      addNotification({
        type: "match_change",
        title: "Match started",
        message: `${p1} vs ${p2} is now live in ${t?.name ?? ""}.`,
        tournamentId,
        matchId,
      });
    },
  ...
  const completeMatch = useCallback(
    (
      ...
      const winner = winnerId
        ? (t?.participants.find((p) => p.id === winnerId)?.name ?? "Unknown")
        : null;
      addNotification({
        type: "match_change",
  ...
  const reportNoShow = useCallback(
    (tournamentId: string, matchId: string, participantId: string) => {
      updateMatch(tournamentId, matchId, { status: "delayed" });
      const t = tournaments.find((x) => x.id === tournamentId);
      const name =
        t?.participants.find((p) => p.id === participantId)?.name ??
        "Participant";
      addNotification({
        type: "no_show",
        title: "No-show reported",
        message: `${name} did not appear. Match rescheduled automatically.`,
        tournamentId,
        matchId,
      });
    },
  ...
  const reportDelay = useCallback(
    (tournamentId: string, matchId: string, minutes: number) => {
      updateMatch(tournamentId, matchId, { status: "delayed" });
      const t = tournaments.find((x) => x.id === tournamentId);
      const m = t?.matches.find((x) => x.id === matchId);
      const p1 =
      ...
      addNotification({
        type: "delay",
        title: `Match delayed ${minutes} min`,
        message: `${p1} vs ${p2} pushed back. Downstream matches auto-adjusted.`,
        tournamentId,
        matchId,
      });
    },
  ...
  const setDelayedMatchTimer = useCallback(
    (matchId: string, timer: DelayedMatchTimer) => {
  ...
  const bookRoom = useCallback(
    (tournamentId: string, matchId: string, locationId: string) => {
      ...
      addNotification({
        type: "room_change",
        title: "Room assigned",
        message: `Match assigned to ${loc?.name ?? "room"} in ${loc?.building ?? ""}.`,
        tournamentId,
        matchId,
      });
    },
