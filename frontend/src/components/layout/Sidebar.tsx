...
import { Trophy, LayoutGrid, Users, Calendar, Bell, Settings, Plus, Zap, MapPin, X, LogOut, Search, Clock } from 'lucide-react';
...
export default function Sidebar({ open, onClose }: SidebarProps) {
  ...
  const { tournaments, notifications } = useApp() || { tournaments: [], notifications: [] };
  ...
  const settingsSections = [
    'Profile', 'Appearance', 'Time & Timezone', 'Notifications', 'Schedule Preferences',
    'Participant Preferences', 'Tournament Defaults',
  ];

  const results = q.length < 1 ? [] : [
    // Dashboard
    ...(['dashboard', 'overview', 'stats', 'home'].some(k => k.includes(q)) ? [{ label: 'Dashboard', sub: 'Overview & stats', icon: LayoutGrid, path: '/dashboard' }] : []),
    // Tournaments
    ...(tournaments ?? []).filter(t =>
      t.name.toLowerCase().includes(q) || t.game.toLowerCase().includes(q) || t.format.toLowerCase().includes(q) || t.status.toLowerCase().includes(q)
    ).map(t => ({ label: t.name, sub: `${t.game} · ${t.status}`, icon: Trophy, path: `/tournaments/${t.id}` })),
    // Participants
    ...(tournaments ?? []).flatMap(t => t.participants.filter(p =>
      p.name.toLowerCase().includes(q) || p.email.toLowerCase().includes(q) || p.status.toLowerCase().includes(q)
    ).map(p => ({ label: p.name, sub: p.email, icon: Users, path: '/participants' }))),
    // Schedule / matches
    ...(tournaments ?? []).flatMap(t => t.matches.filter(m => {
      const p1 = t.participants.find(p => p.id === m.participant1Id)?.name ?? '';
      const p2 = t.participants.find(p => p.id === m.participant2Id)?.name ?? '';
      return p1.toLowerCase().includes(q) || p2.toLowerCase().includes(q) || m.status.toLowerCase().includes(q);
    }).map(m => {
      ...
      return { label: `${p1} vs ${p2}`, sub: `Schedule · ${m.status}`, icon: Calendar, path: '/schedule' };
    })),
    // Live Control — live/delayed matches
    ...(tournaments ?? []).flatMap(t => t.matches.filter(m =>
      (m.status === 'live' || m.status === 'delayed') &&
      (['live', 'delay', 'control'].some(k => k.includes(q)) ||
        (t.participants.find(p => p.id === m.participant1Id)?.name ?? '').toLowerCase().includes(q) ||
        (t.participants.find(p => p.id === m.participant2Id)?.name ?? '').toLowerCase().includes(q))
    ).map(m => {
      const p1 = t.participants.find(p => p.id === m.participant1Id)?.name ?? 'TBD';
      const p2 = t.participants.find(p => p.id === m.participant2Id)?.name ?? 'TBD';
      return { label: `${p1} vs ${p2}`, sub: `Live Control · ${m.status}`, icon: Zap, path: '/live' };
    })),
    // Room Booking — locations
    ...(tournaments ?? []).flatMap(t => t.locations.filter(l =>
      l.name.toLowerCase().includes(q) || l.building.toLowerCase().includes(q)
    ).map(l => ({ label: l.name, sub: `Room Booking · ${l.building}`, icon: MapPin, path: '/rooms' }))),
    // Notifications
    ...(notifications ?? []).filter(n =>
      n.title.toLowerCase().includes(q) || n.message.toLowerCase().includes(q)
    ).map(n => ({ label: n.title, sub: n.message.slice(0, 50), icon: Bell, path: '/notifications' })),
    // Settings sections
    ...settingsSections.filter(s => s.toLowerCase().includes(q))
      .map(s => ({ label: s, sub: 'Settings', icon: Settings, path: '/settings' })),
    // Time blocks
    ...(tournaments ?? []).flatMap(t => t.timeBlocks.filter(tb =>
      tb.label.toLowerCase().includes(q)
    ).map(tb => ({ label: tb.label, sub: `Schedule · ${tb.start}–${tb.end}`, icon: Clock, path: '/schedule' }))),
  ].filter((r, i, arr) => arr.findIndex(x => x.label === r.label && x.path === r.path) === i)
   .slice(0, 8);
    ...
    <aside className={`fixed top-0 bottom-0 left-0 z-50 w-64 glass-panel border-y-0 border-l-0 rounded-none flex flex-col transition-transform duration-300 ease-in-out lg:translate-x-0 ${open ? 'translate-x-0' : '-translate-x-full'}`}>
      ...
      <nav className="flex-1 overflow-y-auto py-4 px-3 flex flex-col gap-1 custom-scrollbar">
        ...
        <div className="relative mb-2" ref={searchRef}>
          <Search size={13} className="absolute left-3 top-1/2 -translate-y-1/2 text-arena-text-muted pointer-events-none" />
          <input
            ...
            <div className="absolute left-0 right-0 top-full mt-1 z-50 bg-arena-surface border border-arena-border rounded-lg shadow-2xl overflow-hidden">
              {results.map((r, i) => (
                <button
                  ...
                  onClick={() => { navigate(r.path); setQuery(''); setShowResults(false); handleNav(); }}
                >
                  <r.icon size={13} className="text-arena-accent flex-shrink-0" />
                  <div className="min-w-0">
                    <p className="text-xs font-semibold text-arena-text truncate">{r.label}</p>
                    <p className="text-xs text-arena-text-muted truncate">{r.sub}</p>
                  </div>
                </button>
              ))}
            </div>
