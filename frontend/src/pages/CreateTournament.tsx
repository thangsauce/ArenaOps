...
function DatePicker({
  ...
  const monthYearRef = useRef<HTMLButtonElement | null>(null);
  ...
  const focusCalendarDay = () => {
    window.setTimeout(() => {
      const selectedDay = dayRefs.current.find((button) =>
        button?.className.includes(styles.daySelected),
      );
      const todayDay = dayRefs.current.find((button) =>
        button?.className.includes(styles.dayToday),
      );
      const firstEnabledDay = dayRefs.current.find((button) => button && !button.disabled);
      selectedDay?.focus() ?? todayDay?.focus() ?? firstEnabledDay?.focus();
    }, 0);
  };
              ...
              return (
                <button
                  ...
                  onKeyDown={(e) => {
                    ...
                    const step = movementMap[e.key];
                    if (!step) return;
                    e.preventDefault();
                    let nextIndex = i + step;
                    while (nextIndex >= 0 && nextIndex < cells.length) {
                      const nextButton = dayRefs.current[nextIndex];
...
export default function CreateTournament() {
  ...
  const gameCategoryRefs = useRef<Array<HTMLButtonElement | null>>([]);
  const gameOptionRefs = useRef<Array<HTMLButtonElement | null>>([]);
  const formatRefs = useRef<Array<HTMLButtonElement | null>>([]);
  const customGameInputRef = useRef<HTMLInputElement | null>(null);
  const continueButtonRef = useRef<HTMLButtonElement | null>(null);
  const maxParticipantsRef = useRef<HTMLSelectElement | null>(null);
  const startDateTriggerRef = useRef<HTMLButtonElement | null>(null);
  const organizerNameRef = useRef<HTMLInputElement | null>(null);
  const descriptionRef = useRef<HTMLTextAreaElement | null>(null);
  const stepTwoCancelRef = useRef<HTMLButtonElement | null>(null);
  ...
  const focusNearestGridButton = (
    refs: MutableRefObject<Array<HTMLButtonElement | null>>,
    currentIndex: number,
    key: 'ArrowLeft' | 'ArrowRight' | 'ArrowUp' | 'ArrowDown',
  ): boolean => {
    const buttons = refs.current.filter(
      (button): button is HTMLButtonElement => button !== null,
    );
    const currentButton = refs.current[currentIndex];
    if (!currentButton || buttons.length === 0) return false;
    ...
    const candidates = buttons
      ...
      .filter(({ deltaX, deltaY }) => {
        if (key === 'ArrowLeft') return deltaX < -8 && Math.abs(deltaY) < currentRect.height * 0.75;
        if (key === 'ArrowRight') return deltaX > 8 && Math.abs(deltaY) < currentRect.height * 0.75;
        if (key === 'ArrowUp') return deltaY < -8;
        return deltaY > 8;
      })
      .sort((a, b) => {
        if (key === 'ArrowLeft' || key === 'ArrowRight') {
          const distanceA = Math.abs(a.deltaX);
          const distanceB = Math.abs(b.deltaX);
          if (distanceA !== distanceB) return distanceA - distanceB;
          return Math.abs(a.deltaY) - Math.abs(b.deltaY);
        }

        const verticalA = Math.abs(a.deltaY);
        const verticalB = Math.abs(b.deltaY);
        if (verticalA !== verticalB) return verticalA - verticalB;
        return Math.abs(a.deltaX) - Math.abs(b.deltaX);
      });
  ...
  const handleGridNavigation = (
    ...
    if (e.key === 'ArrowLeft' || e.key === 'ArrowRight') {
      const validIndexes = refs.current
        .map((button, buttonIndex) => (button ? buttonIndex : -1))
        .filter((buttonIndex) => buttonIndex !== -1);
      const currentValidIndex = validIndexes.indexOf(index);
      if (currentValidIndex === -1 || validIndexes.length === 0) return;
      const nextValidIndex =
        e.key === 'ArrowRight'
          ? (currentValidIndex + 1) % validIndexes.length
          : (currentValidIndex - 1 + validIndexes.length) % validIndexes.length;
      refs.current[validIndexes[nextValidIndex]]?.focus();
      return;
    }

    const moved = focusNearestGridButton(
      refs,
      index,
      e.key as 'ArrowLeft' | 'ArrowRight' | 'ArrowUp' | 'ArrowDown',
    );
    if (!moved && e.key === 'ArrowDown') {
      options?.onDownBoundary?.();
    }
  };

  const handleGameCategoryKeyDown = (
    e: KeyboardEvent<HTMLButtonElement>,
    index: number,
  ) => {
    if (e.key === 'Enter') {
      e.preventDefault();
      const selectedCategory = GAME_CATALOG[index];
      setGameCategory(selectedCategory.category);
      set('game', '');
      window.setTimeout(() => {
        gameOptionRefs.current[0]?.focus();
      }, 0);
      return;
    }
    ...
    if (e.key === 'ArrowRight' || e.key === 'ArrowDown') {
      nextIndex = (index + 1) % GAME_CATALOG.length;
    }
    if (e.key === 'ArrowLeft' || e.key === 'ArrowUp') {
      nextIndex = (index - 1 + GAME_CATALOG.length) % GAME_CATALOG.length;
    }
    const nextCategory = GAME_CATALOG[nextIndex];
    setGameCategory(nextCategory.category);
    set('game', '');
    gameCategoryRefs.current[nextIndex]?.focus();
  };
  ...
  const focusStepThreeDraft = () => {
    window.setTimeout(() => {
      stepThreeDraftRef.current?.focus();
    }, 0);
  };
        ...
        <div className={styles.stepContent}>
            ...
            <label>Tournament Name</label>
              <input
                ...
                onKeyDown={e => {
                  ...
                  e.preventDefault();
                  const firstCategory = GAME_CATALOG[0];
                  setGameCategory(current => current || firstCategory.category);
                  gameCategoryRefs.current[0]?.focus();
                }}
            />
          </div>
          ...
          <div className={styles.field}>
            ...
            <div className={styles.gameCategoryTabs}>
              {GAME_CATALOG.map(({ category, Icon }, index) => (
                <button
                  ref={(node) => {
                    gameCategoryRefs.current[index] = node;
                  }}
                  key={category}
                  type="button"
                  ...
                  onKeyDown={(e) => handleGameCategoryKeyDown(e, index)}
                  role="tab"
                  aria-selected={gameCategory === category}
                  tabIndex={gameCategory === category || (!gameCategory && index === 0) ? 0 : -1}
                >
            ...
            {gameCategory && (
              <>
                ...
                <div className={styles.gameGrid}>
                  {GAME_CATALOG.find(c => c.category === gameCategory)?.games.map((game, index) => (
                    <button
                      ref={(node) => {
                        gameOptionRefs.current[index] = node;
                      }}
                      key={game.name}
                      type="button"
                      ...
                      onKeyDown={(e) =>
                        handleGridNavigation(e, gameOptionRefs, index, {
                          onEnter: () => {
                            set('game', game.name);
                            window.setTimeout(() => {
                          ...
                          onDownBoundary: () => {
                            customGameInputRef.current?.focus();
                          },
                        })}
                    >
            ...
            <div className={styles.gameCustomRow}>
              ...
              <input
                ...
                onKeyDown={e => {
                  if (e.key === 'ArrowUp') {
                    e.preventDefault();
                    const validIndexes = gameOptionRefs.current
                      .map((button, buttonIndex) => (button ? buttonIndex : -1))
                      .filter((buttonIndex) => buttonIndex !== -1);
                    const lastIndex = validIndexes[validIndexes.length - 1];
                    if (lastIndex !== undefined) {
                      gameOptionRefs.current[lastIndex]?.focus();
                    }
                    return;
                  }
                  if (e.key !== 'Enter') return;
          ...
          <div className={styles.field}>
            ...
            <div className={styles.formatGrid}>
              {formats.map((f, index) => (
                <button
                  ref={(node) => {
                    formatRefs.current[index] = node;
                  }}
                  key={f.value}
                  className={`${styles.formatCard} ${form.format === f.value ? styles.selected : ''}`}
                  onClick={() => set('format', f.value)}
                  onKeyDown={(e) =>
                    handleGridNavigation(e, formatRefs, index, {
                      onEnter: () => {
                        set('format', f.value);
                        window.setTimeout(() => {
          ...
          <button
            ...
            onKeyDown={(e) => {
              if (e.key !== 'Enter' || !canNext1) return;
              e.preventDefault();
              setStep(2);
              window.setTimeout(() => {
                maxParticipantsRef.current?.focus();
        ...
        <div className={styles.stepContent}>
          ...
          <div className={styles.row}>
            ...
            <div className={styles.field}>
              ...
              <DatePicker
                ...
                onDateSelected={() => organizerNameRef.current?.focus()}
              />
            </div>
          </div>
          ...
          <div className={styles.field}>
            ...
            <input
              ...
              onKeyDown={e => {
                if (e.key !== 'Enter') return;
                if (!form.organizerName.trim()) return;
                e.preventDefault();
          ...
          <div className={styles.field}>
            ...
            <textarea
              ...
              onKeyDown={e => {
                ...
                e.preventDefault();
                stepTwoContinueRef.current?.focus();
              }}
              rows={3}
            />
          </div>
          ...
          <div className={styles.btnRow}>
            ...
            <button
              ...
              onKeyDown={e => {
                if (['ArrowLeft', 'ArrowRight', 'ArrowUp', 'ArrowDown'].includes(e.key)) {
                  e.preventDefault();
                  stepTwoCancelRef.current?.focus();
                  return;
                }
                if (e.key === 'Enter' && canNext2) {
                  e.preventDefault();
                  setStep(3);
                  focusStepThreeDraft();
                }
      ...
      {step === 3 && (
        <div className={styles.stepContent}>
          ...
          <div className={styles.actionRow}>
            <button
              ref={stepThreeCancelRef}
              className={styles.cancelBtn}
              onClick={() => navigate('/dashboard')}
              onKeyDown={(e) => {
                handleStepThreeNav(e, 'cancel');
                if (e.key === 'Enter') {
                  e.preventDefault();
                  navigate('/dashboard');
                }
