...
import { Zap, Trophy, Calendar, MapPin, Users, ArrowRight, Shield, Gamepad2, Github, Mail, Globe } from 'lucide-react';
import { motion } from 'framer-motion';
import type { Variants } from 'framer-motion';

const games = [
  { name: 'VALORANT',          genre: 'Tactical FPS',  format: '5v5',    color1: '#1a0508', color2: '#cc1828', accent: '#ff4655' },
  { name: 'LEAGUE OF LEGENDS', genre: 'MOBA',          format: '5v5',    color1: '#020a14', color2: '#0d2a4a', accent: '#C89B3C' },
  { name: 'CS2',               genre: 'FPS',           format: '5v5',    color1: '#08101c', color2: '#162244', accent: '#4a9eff' },
  { name: 'ROCKET LEAGUE',     genre: 'Sports',        format: '3v3',    color1: '#08001a', color2: '#200060', accent: '#00d4ff' },
  { name: 'OVERWATCH 2',       genre: 'Hero Shooter',  format: '5v5',    color1: '#1a0c00', color2: '#502800', accent: '#ff6b00' },
  { name: 'APEX LEGENDS',      genre: 'Battle Royale', format: '3v3',    color1: '#100000', color2: '#3a0800', accent: '#fc4d2c' },
  { name: 'SMASH BROS',        genre: 'Fighting',      format: '1v1',    color1: '#0e0210', color2: '#38003c', accent: '#ff2244' },
  { name: 'STREET FIGHTER 6',  genre: 'Fighting',      format: '1v1',    color1: '#0a0010', color2: '#2a0050', accent: '#cc00ff' },
  { name: 'FORTNITE',          genre: 'Battle Royale', format: 'Squads', color1: '#001028', color2: '#002060', accent: '#00b4ff' },
  { name: 'TEKKEN 8',          genre: 'Fighting',      format: '1v1',    color1: '#100008', color2: '#3c0020', accent: '#ff0066' },
];

const features = [
  { icon: Trophy,   title: 'Tournament Management', desc: 'Create and run single-elimination, round-robin, and double-elimination brackets with ease.' },
  { icon: Zap,      title: 'Live Match Control',     desc: 'Start matches, report scores, handle no-shows, and push real-time updates to all participants.' },
  { icon: Calendar, title: 'Smart Scheduling',       desc: 'Collect availability, auto-generate conflict-free schedules, and send confirmation links.' },
  { icon: MapPin,   title: 'Room Booking',           desc: 'Assign venues, manage time slots, and prevent double-bookings across all your events.' },
  { icon: Users,    title: 'Participant Tracking',   desc: 'Register teams and players, track check-ins, and manage waitlists automatically.' },
  { icon: Shield,   title: 'Role-Based Access',      desc: 'Give organizers, referees, and volunteers the exact permissions they need — nothing more.' },
];
...
function MarqueeCard({ game }: { game: Game }) {
  return (
    <div
      className="relative flex-shrink-0 w-52 h-32 rounded-2xl overflow-hidden border border-white/10 cursor-pointer group select-none"
      style={{ background: `linear-gradient(135deg, ${game.color1} 0%, ${game.color2} 100%)` }}
    >
      {/* Accent glow */}
      <div
        className="absolute inset-0"
        style={{ background: `radial-gradient(ellipse at 75% 25%, ${game.accent}55 0%, transparent 65%)` }}
      />
      {/* Corner bloom */}
      <div
      ...
      <div className="absolute inset-0 p-4 flex flex-col justify-end">
        <p className="text-[10px] font-bold tracking-[0.15em] uppercase mb-1 opacity-80" style={{ color: game.accent }}>
          {game.format} · {game.genre}
        </p>
        <p className="font-display text-white text-lg leading-tight">{game.name}</p>
      </div>
...
function GameCard({ game }: { game: Game }) {
  return (
    ...
    >
      ...
      <div
        className="absolute inset-0"
        style={{ background: `radial-gradient(ellipse at 80% 15%, ${game.accent}45 0%, transparent 60%)` }}
      />
      {/* Corner bloom */}
      <div
      ...
      <div className="absolute bottom-0 left-0 right-0 p-4">
        <div className="flex items-center gap-2 mb-1.5">
          <span
            ...
            style={{ borderColor: `${game.accent}55`, color: game.accent, backgroundColor: `${game.accent}18` }}
          >
            {game.genre}
          </span>
        ...
        <h3 className="font-display text-white text-2xl leading-tight">{game.name}</h3>
      </div>
      {/* Hover overlay CTA */}
      ...
      <section className="relative py-20 z-10">
        ...
        >
          ...
          <p className="text-arena-text-muted text-lg max-w-xl mx-auto">
            From FPS to fighting games — run professional tournaments for every major esports title.
          </p>
        </motion.div>
        ...
        >
          <div className="flex gap-4 w-max marquee-track animate-marquee-right px-4">
            {[...[...games].reverse(), ...[...games].reverse()].map((game, i) => (
              <MarqueeCard key={i} game={game} />
            ))}
          </div>
        </div>
        ...
        <div className="border-t border-arena-border bg-arena-surface/20 backdrop-blur-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
              ...
              <div className="md:col-span-2 lg:col-span-1">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <span className="font-display tracking-wide text-xl font-bold">ArenaOPS</span>
                </div>
                <p className="text-sm text-arena-text-muted leading-relaxed mb-6 max-w-xs">
                  The all-in-one tournament platform built for university esports clubs — from first registration to final bracket.
                </p>
              ...
              {/* Platform links */}
              <div>
                <h4 className="font-display text-xs uppercase tracking-[0.15em] text-arena-text font-bold mb-5">Platform</h4>
                <ul className="space-y-3">
                  {['Tournament Management', 'Live Match Control', 'Smart Scheduling', 'Room Booking', 'Participant Tracking', 'Role-Based Access'].map(item => (
                    <li key={item}>
                      <a href="#" className="text-sm text-arena-text-muted hover:text-arena-accent transition-colors duration-150">{item}</a>
                    </li>
                  ))}
                </ul>
              </div>
              ...
              {/* Resources */}
              <div>
                <h4 className="font-display text-xs uppercase tracking-[0.15em] text-arena-text font-bold mb-5">Resources</h4>
                <ul className="space-y-3">
                  {['Documentation', 'API Reference', 'Changelog', 'System Status', 'Support Center', 'Community'].map(item => (
                    <li key={item}>
                      <a href="#" className="text-sm text-arena-text-muted hover:text-arena-accent transition-colors duration-150">{item}</a>
                    </li>
                  ))}
              ...
              {/* Legal */}
              <div>
                <h4 className="font-display text-xs uppercase tracking-[0.15em] text-arena-text font-bold mb-5">Legal</h4>
                <ul className="space-y-3">
          ...
          <div className="border-t border-arena-border">
            <div className="max-w-6xl mx-auto px-6 py-5 flex flex-col sm:flex-row items-center justify-between gap-3">
              <p className="text-xs text-arena-text-muted">© 2026 ArenaOPS · All rights reserved</p>
              <div className="flex items-center gap-1.5 text-xs text-arena-text-muted">
                <span>Free for university clubs</span>
                <span className="text-arena-border">·</span>
                <span className="text-arena-accent font-semibold">No credit card required</span>
              </div>
            </div>
          </div>
        </div>
