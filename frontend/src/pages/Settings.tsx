...
function Dropdown<T extends string>({
  ...
  return (
    <div
      className={`relative w-full max-w-sm ${open ? "z-50" : "z-10"}`}
      ref={ref}
    >
      <button
      ...
      >
        ...
        <ChevronDown
          size={16}
          className={`text-arena-text-muted transition-transform duration-200 ${open ? "rotate-180 text-arena-accent" : ""}`}
        />
      </button>
      ...
      <AnimatePresence>
        {open && (
          ...
          >
            <div className="max-h-64 overflow-y-auto custom-scrollbar" role="listbox">
              {options.map((opt, index) => (
                <button
                  key={opt.value}
                  role="option"
                  aria-selected={opt.value === value}
                  className={`w-full text-left px-4 py-3 flex items-center justify-between hover:bg-arena-surface-hover transition-colors border-b border-arena-border last:border-0 ${index === focusedIndex ? 'bg-arena-surface-hover' : ''}`}
                  onClick={() => {
                    onChange(opt.value);
                    setOpen(false);
                  }}
                >
                  <div className="flex flex-col">
                    <span
                      className={`text-sm font-semibold ${opt.value === value ? "text-arena-accent" : "text-arena-text"}`}
                    >
                      {opt.label}
                    </span>
                    {opt.sub && (
                      <span className="text-xs text-arena-text-muted mt-0.5">
                        {opt.sub}
                      </span>
                    )}
                  </div>
                  {opt.value === value && (
                    <Check size={16} className="text-arena-accent" />
                  )}
                </button>
              ))}
            </div>
  ...
  return (
    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 data-[density=compact]:gap-2 py-3 data-[density=compact]:py-2 w-full">
      <div className="max-w-md">
        ...
        {sub && (
          <p className="text-xs text-arena-text-muted mt-1 leading-relaxed">
            {sub}
          </p>
        )}
      </div>
...
export default function Settings() {
  ...
  const settings = state?.settings || {
    ...
    tournamentDefaults: {
      format: "single-elimination" as DefaultFormat,
      maxParticipants: "16" as DefaultMaxParticipants,
      organizerName: "Admin",
    },
    schedulePrefs: {
      defaultView: "grid" as DefaultScheduleView,
      weekStart: "monday" as WeekStart,
    },
    bracketPrefs: { defaultView: "tree" as DefaultBracketView },
    participantPrefs: {
      sortBy: "seed" as ParticipantSort,
      hideDeclined: false,
    },
  ...
  const saveProfile = () => {
    ...
    updateSettings({
      tournamentDefaults: {
        ...tournamentDefaults,
        organizerName: profileForm.name,
      },
    });
    setSaved(true);
    toast('Profile saved');
  ...
  const currentTz =
    TIMEZONES.find(
      (t: (typeof TIMEZONES)[0]) => t.value === timePrefs.timezone,
    ) || TIMEZONES[0];
  ...
  return (
    <div className="w-full px-10 pt-8 pb-12">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-6 pb-5 border-b border-arena-border">
        <div>
          <h1 className="font-display text-4xl font-bold uppercase tracking-tight text-arena-text">
            Settings
          </h1>
          <p className="text-arena-text-muted text-xs font-medium mt-2">
            Customize your ArenaOPS experience
          </p>
        </div>
      </div>
      ...
      <Section
        icon={User}
        title="Profile"
        desc="Your organizer identity across all tournaments"
        defaultOpen={false}
        zIndex={70}
      >
        ...
        <Row label="University" sub="Shown on your tournament pages">
          <input
            ...
            onChange={(e) =>
              setProfileForm((p) => ({ ...p, university: e.target.value }))
            }
          />
        </Row>
      ...
      >
        <Row
          label="Theme"
          sub="Controls the color scheme across the app. Automatically syncs with AppLayout."
        >
          <Dropdown<Theme>
            value={appearance.theme}
            onChange={(theme) =>
              updateSettings({ appearance: { ...appearance, theme } })
            }
            options={[
        ...
        <Row label="Density" sub="Controls row spacing in tables and lists">
          <ChipGroup<Density>
            ...
            onChange={(density) =>
              updateSettings({ appearance: { ...appearance, density } })
            }
            options={[
        ...
        <Row
          label="Side Menu (Mobile)"
          sub="Place the menu button on the left or right for easier reach"
        >
          <ChipGroup<"left" | "right">
            value={appearance.mobileMenuPosition}
            onChange={(mobileMenuPosition) =>
              updateSettings({
                appearance: { ...appearance, mobileMenuPosition },
              })
            }
            options={[
      ...
      <Section
        icon={Clock}
        title="Time & Timezone"
        desc="How times appear across Schedule and match views"
        defaultOpen={false}
        zIndex={50}
      >
        <Row
        ...
        >
          <ChipGroup<TimeFormat>
            ...
            onChange={(format) =>
              updateSettings({ timePrefs: { ...timePrefs, format } })
            }
            options={[
        ...
        <Row
          label="Timezone"
          sub={`All match times converted to this zone · Active: ${currentTz.label} (${currentTz.abbr})`}
        >
          <Dropdown<Timezone>
            value={timePrefs.timezone}
            onChange={(timezone) =>
              updateSettings({ timePrefs: { ...timePrefs, timezone } })
            }
            options={TIMEZONES.map((tz: (typeof TIMEZONES)[0]) => ({
              value: tz.value,
              label: tz.label,
              sub: tz.abbr,
            }))}
      ...
      <Section
        icon={Bell}
        title="Notifications"
        desc="Choose which events trigger in-app notifications"
        defaultOpen={false}
        zIndex={40}
      >
        {(
          [
            {
              key: "matchStart",
              label: "Match started",
              sub: "When a match goes live",
            },
            {
              key: "delays",
              label: "Match delays",
              sub: "When a match is pushed back",
            },
            {
              key: "noShows",
              label: "No-shows",
              sub: "When a participant is marked absent",
            },
            {
              key: "roomChanges",
              label: "Room changes",
              sub: "When a location is reassigned",
            },
          ] as { key: keyof typeof notifications; label: string; sub: string }[]
        ).map(({ key, label, sub }) => (
          <Row key={key} label={label} sub={sub}>
            <Toggle
              ...
              onChange={(v) =>
                updateSettings({
                  notifications: { ...notifications, [key]: v },
                })
              }
            />
          </Row>
        ...
        <Row label="Sound alerts" sub="Play a sound when notifications arrive">
          <div className="flex items-center gap-3">
            {notifications.sound ? (
              ...
              <VolumeX size={16} className="text-arena-text-muted" />
            )}
            <Toggle
              checked={notifications.sound}
              onChange={(v) =>
                updateSettings({
                  notifications: { ...notifications, sound: v },
                })
              }
            />
          </div>
      ...
      <Section
        ...
        desc="Default view and layout for the schedule page"
        defaultOpen={false}
        zIndex={38}
      >
        <Row
          label="Default view"
          sub="How the schedule is displayed when you open it"
        >
          <ChipGroup<DefaultScheduleView>
            value={schedulePrefs.defaultView}
            onChange={(defaultView) =>
              updateSettings({
                schedulePrefs: { ...schedulePrefs, defaultView },
              })
            }
            options={[
        ...
        <Row
          ...
          sub="First day shown in weekly schedule views"
        >
          <ChipGroup<WeekStart>
            value={schedulePrefs.weekStart}
            onChange={(weekStart) =>
              updateSettings({ schedulePrefs: { ...schedulePrefs, weekStart } })
            }
            options={[
      ...
      <Section
        icon={Trophy}
        title="Bracket Preferences"
        desc="Choose how tournament brackets open by default"
        defaultOpen={false}
        zIndex={37}
      >
        <Row
          label="Default bracket view"
          sub="How brackets are shown when you open a tournament"
        >
          <ChipGroup<DefaultBracketView>
            value={bracketPrefs.defaultView}
            onChange={(defaultView) =>
              updateSettings({ bracketPrefs: { defaultView } })
            }
            options={[
      ...
      <Section
        ...
        desc="Control how participants are displayed and filtered"
        defaultOpen={false}
        zIndex={35}
      >
        <Row
          label="Sort participants by"
          sub="Default ordering in participant lists"
        >
          <Dropdown<ParticipantSort>
            value={participantPrefs.sortBy}
            onChange={(sortBy) =>
              updateSettings({
                participantPrefs: { ...participantPrefs, sortBy },
              })
            }
            options={[
        ...
        <Row
          label="Hide declined participants"
          sub="Remove declined entries from participant lists"
        >
          <Toggle
            checked={participantPrefs.hideDeclined}
            onChange={(v) =>
              updateSettings({
                participantPrefs: { ...participantPrefs, hideDeclined: v },
              })
            }
          />
        </Row>
      ...
      <Section
        ...
        desc="Pre-fill values when creating a new tournament"
        defaultOpen={false}
        zIndex={30}
      >
        <Row
          label="Default format"
          sub="Pre-selected when opening Create Tournament"
        >
          <Dropdown<DefaultFormat>
            value={tournamentDefaults.format}
            onChange={(format) =>
              updateSettings({
                tournamentDefaults: { ...tournamentDefaults, format },
              })
            }
            options={[
        ...
        <Row label="Default max participants">
          <Dropdown<DefaultMaxParticipants>
            ...
            onChange={(maxParticipants) =>
              updateSettings({
                tournamentDefaults: { ...tournamentDefaults, maxParticipants },
              })
            }
            options={["4", "8", "16", "32", "64"].map((n) => ({
              value: n as DefaultMaxParticipants,
              label: `${n} players`,
            }))}
          />
        </Row>
