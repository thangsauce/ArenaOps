...
function MatchScore({
  ...
  return (
    <div className={`${styles.scoreControl} ${large ? styles.scoreControlLarge : ""}`}>
      <input
        type="text"
        inputMode="numeric"
        pattern="[0-9]*"
...
export default function LiveControl() {
  const { tournaments, timePrefs, startMatch, completeMatch, reportNoShow, reportDelay } =
    useApp();
  ...
  const [liveScores, setLiveScores] = useState<Record<string, { score1: number; score2: number }>>({});
  ...
  const formatSlotTime = (slot?: { start: string; date: string }) =>
    slot ? formatTime(slot.start, slot.date, timePrefs.format, timePrefs.timezone) : null;
  ...
  const getLiveScores = (matchId: string, initialScore1 = 0, initialScore2 = 0) =>
  ...
  const openScoreModal = (matchId: string) => {
    ...
    if (!match) return;
    const currentScores = getLiveScores(matchId, match.score1 ?? 0, match.score2 ?? 0);
      ...
      <section className={styles.section}>
        ...
        <div className={styles.matchGrid}>
          {live.map((m) => {
            ...
            const currentScores = getLiveScores(m.id, m.score1 ?? 0, m.score2 ?? 0);
            return (
                ...
                <div className={styles.matchup}>
                  <div className={styles.player}>
                    ...
                    <MatchScore
                      ...
                      onChange={(value) =>
                        updateLiveScore(m.id, "score1", value, m.score1 ?? 0, m.score2 ?? 0)
                      }
                    />
                  </div>
                  ...
                  <div className={styles.player}>
                    ...
                    <MatchScore
                      ...
                      onChange={(value) =>
                        updateLiveScore(m.id, "score2", value, m.score1 ?? 0, m.score2 ?? 0)
                      }
                    />
                  </div>
                </div>
                ...
                <div key={m.id} className={styles.doneRow}>
                  ...
                  <div className={styles.donePlayers}>
                    <div className={styles.donePlayerBlock}>
                      ...
                      <span className={`${styles.doneScoreBox} ${!isTie && (m.score1 ?? 0) > (m.score2 ?? 0) ? styles.doneScoreBoxWinner : ''}`}>
                        {m.score1 ?? 0}
                      </span>
                    </div>
                    ...
                    <div className={styles.donePlayerBlock}>
                      ...
                      <span className={`${styles.doneScoreBox} ${!isTie && (m.score2 ?? 0) > (m.score1 ?? 0) ? styles.doneScoreBoxWinner : ''}`}>
                        {m.score2 ?? 0}
                      </span>
                    </div>
                  </div>
                ...
                {score1 !== "" &&
                  ...
                  (() => {
                    const s1 = parseInt(score1),
                      s2 = parseInt(score2);
                    return s1 === s2 ? (
                      <p className={styles.winnerPreview}>Result: <strong style={{ color: "var(--text)" }}>Tie</strong></p>
                    ) : (
      ...
      {delayModal && (
        <div className={styles.overlay} onClick={() => setDelayModal(null)}>
          <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
            ...
            <div className={styles.delayOptions}>
              {[5, 10, 15, 30, 60].map((mins) => (
                <button
                  ...
                  onClick={() => {
                    reportDelay(tournament.id, delayModal.matchId, mins);
                    setDelayModal(null);
                  }}
                >
            ...
            <p className={styles.autoNote}>
              ⚡ Downstream matches will be automatically adjusted and
